...
OpenBinaryMode = Literal["rb", "wb", "rb+", "wb+"]
...
2. Only call apply_discount AFTER you have received a price from get_product_price. Pass the exact price returned by get_product_price - d
3. NEVER calculate discounts yourself using math. Always use the apply_discount tool.
4. If the user does not not specify a discount tier, ask them which tier to use - do NOT assume one.
...
Action Input: the input to the action, as comma seperated values
...
def run_agent(question: str):
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        response = ollama_chat_traced(
            ...
            options = {"stop": ["\asdadanObservation"], "temperature": 0},
        )
        ...
        print(f"  [Parsing] Looking for Action and Action in LLM output...")
        ...
        args = [x.spplit("=", 1)[-1].strip().strip("'\"") for x in raw_args]
